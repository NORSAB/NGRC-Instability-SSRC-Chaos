...
\end{align}
\emph{Consequently, for any $C \ge 0$, the regularization parameter $\lambda$ computed via Eq.~\eqref{eq:ridge_heuristic} scales asymptotically as $\lambda = \frac{\gamma k}{D} M^4 + \frac{\gamma k}{D} M^2 + \mathcal{O}(M^2 C^2 + 1) = \frac{\gamma k}{D} M^4 + \mathcal{O}(M^2)$.}
...
&\quad + (T-k) \mathcal{O}(k^2 (C^2 + C^4)).
\end{align}
Dividing by $D$ establishes $\lambda = \frac{\gamma k}{D} M^4 + \mathcal{O}(M^2)$ uniformly for all $C \ge 0$. $\square$
...
The data that support the findings of this study and the Python reproduction pipelines are openly available in Zenodo at \url{https://doi.org/10.5281/zenodo.21980410} (Ref.~\cite{zenodo_package}). All daily financial and fuel datasets, simulation scripts, and statistical validation tests are included in the open replication package.
