...
### Figure 3: Multi-Step Forecasting Horizon Degradation Under Shock (`fig3_horizon_decay`)
Semi-log plot of multi-step Mean Absolute Scaled Error (MASE) across forecasting horizons $\tau \in [0, 1.8]$ ($H = 1$ to $40$) for Lorenz63 under $+15\sigma$ shocks. Regularized readouts (L1, SCAD, Elastic Net) retain forecast accuracy significantly longer than OLS and Ridge.
...
### Figure S3: Realized vs Forecast Volatility During May 2020 Oil Shock (`figS3_fuel_volatility_may2020`)
Bar chart comparing realized daily volatility against out-of-sample forecasts for May 11, 2020, during the negative oil pricing shock. Unconstrained linear readouts produce unphysical negative volatility values, while non-negative regularized methods successfully preserve strict variance positivity.
...
### Table I: Lorenz63 Ablation Benchmark (`table1_ablation`)
Ablation study reporting clean MASE, shocked MASE under $+15\sigma$ perturbations, pairwise win rates, and 95\% bootstrap confidence intervals across ten readout configurations on the Lorenz63 system. Penalized methods consistently outperform unregularized OLS and standard Ridge regression.

### Table II: Financial Volatility Benchmark (`table2_volatility`)
Empirical volatility forecasting benchmark reporting median QLIKE loss and the percentage of negative variance estimates across nine foreign exchange and cryptocurrency time series. Non-negative penalized readouts maintain zero invalid forecasts and minimize divergence compared to unconstrained models.

### Table S1: BCIE Loan Portfolio Forecasting Performance (`tableS1_bcie_comparison`)
Performance comparison table reporting out-of-sample MASE and Diebold-Mariano test p-values relative to baseline models across eight financial forecasting architectures on the BCIE multilateral lending portfolio. Regularized and non-negative configurations achieve statistically significant forecast gains.
...
Mechanistic evaluation table comparing clean and shocked MASE, shock amplification ratios, and forecast retention horizons across six candidate readout mechanisms on Lorenz63 and Rössler attractors. Demonstrates structural stability gains achieved through targeted L1 and sparsity penalties.
